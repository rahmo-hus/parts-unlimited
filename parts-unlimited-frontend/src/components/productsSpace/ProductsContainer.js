        ...
        if(this.props.productsList) {
            ...
            table.map((i) => {
                return (<li key={i} className={this.state.activePage === i ? 'active' : ''}
                            onClick={() => this.setActivePage(i)}><a>{i}</a></li>);
            })
            return <div className="col-sm-8 col-md-9 col-lg-9">
                <div className="product product-grid">
                    ...
                    <div className="row">
                        <ProductsList displayType={this.state.displayType}
                                      productslist={this.props.productsList.slice((this.state.activePage - 1) * this.state.pageSize, (this.state.activePage - 1) * this.state.pageSize + this.state.pageSize)}/>
                    </div>
                    <nav aria-label="Page navigation">
                        <ul className="pagination ht-pagination">
                            <li>
                                <a aria-label="Previous"
                                   style={this.state.activePage === 1 ? {display: 'none'} : {display: 'block'}}>
                      <span aria-hidden="true" onClick={() => {
                        ...
                        <i className="fa fa-chevron-left"/>
                            ...
                            <li>
                                <a aria-label="Next"
                                   style={this.state.activePage === Math.ceil(this.props.productsList.length / this.state.pageSize) ? {display: 'none'} : {display: 'block'}}>
                      <span aria-hidden="true" onClick={() => {
                          this.setActivePage(this.state.activePage + 1)
                      }}>
